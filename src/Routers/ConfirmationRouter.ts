...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    if (!profesores || !asignaturas || !rol) {
      return res
        .status(400)
        .json({ error: "Faltan datos requeridos para la asignación." });
    }
    const result = await service.assign(profesores, asignaturas, rol);
    res
      .status(201)
      .json({ message: "Asignación realizada exitosamente", result });
  })
);
...
router.delete(
  ...
  catchAsync(async (req, res) => {
    ...
    if (!profesorId || !asignaturaId) {
      return res
        .status(400)
        .json({ error: "Faltan datos requeridos para la desvinculación." });
    }
