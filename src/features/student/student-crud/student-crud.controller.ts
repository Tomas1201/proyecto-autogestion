...
import { AssignStudentSubjectSchema } from "../student-validation.middleware.js";
...
export const studentController = {
  ...
  registerStudentToSubject: async (req: Request, res: Response) => {
    try {
      const validatedData = AssignStudentSubjectSchema.parse(req.body);
      const registration = await StudentService.registerStudentToSubject(
        validatedData
      );
      res.status(201).json({
        status: 201,
