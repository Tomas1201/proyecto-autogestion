...
export const AlumnoController = {
  ...
  getAlumno: async (req: Request, res: Response, next:NextFunction) => {
    ...
    try {
      if (!id || isNaN(Number(id))) {
        res.status(400).json({status: 400, message: 'Invalid ID format', ERROR_CODE: 400 });
        return;
      }
      const alumno = await AlumnoService.getById(Number(id));
      if (!alumno) {
        res.status(404).json({status: 404, message: 'Alumno not found', ERROR_CODE: 404 });
        return;
      }
  ...
  getAlumnoByName: async (req: Request, res: Response) => {  
    ...
    try {
      ...
      if (!alumnos || alumnos.length === 0) {
         res.status(404).json({status:404, message: 'No alumnos found with that name' });
         return;
      }
  ...
  getAlumnoByApellido: async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!alumnos || alumnos.length === 0) {
        res.status(404).json({status:404, message: 'No alumnos found with that apellido' });
        return;
      }
      res.status(200).json({status:200, data: alumnos});
      return;
  ...
  createAlumno: async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!newAlumno) {
        res.status(400).json({status:400, message: 'Alumno with the same email, dni or legajo already exists' });
        return;
      }
 ...
 createAlumnoAsignatura: async (req: Request, res: Response) => {
    ...
    try {
      const newinscripcion = await AlumnoService.createAsignaturainscripcion(alumnoid ,asignaturaid);
