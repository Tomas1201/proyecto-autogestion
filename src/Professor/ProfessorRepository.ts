...
export class ProfessorRepository {
  ...
  async findByNombreYDia(nombre: string, dia: string) {
    return await AsignaturaModel.findOne({
      where: {
        nombre,
        dia,
      },
  ...
  async archiveProfessorById(professorId: number) {
    ...
    if (!profesor) throw new Error("Profesor no encontrado");
    if (profesor.get("state") !== true)
      throw new Error("El profesor ya está archivado");
